...
class EdgeSplitPair(Variable["SplitPair"]):
    ...
    those outgoing edges ``(e1, e2)``, this variable is 1 iff both edges are
    selected (i.e., the pair is active as a split).
    ...
    :math:`g_{e_1,e_2}` is the split group indicator.
    """
    ...
    def instantiate(solver: Solver) -> Collection[SplitPair]:
        ...
        for node in solver.graph.nodes:
            ...
            if len(next_edges) >= 2:
                for e1, e2 in combinations(next_edges, 2):
                    groups.append((e1, e2))
        return groups
